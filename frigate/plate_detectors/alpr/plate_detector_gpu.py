...
from retina_plate.models.retinaplate import RetinaPlate
...
class Plate_Detector:
    def __init__(self):
        ...
        priorbox = PriorBox(self.config, image_size=(self.image_size[1], self.image_size[0]),
    ...
    def get_input(self, img, debug=False):
        ...
        if (im_width / im_height) != (self.config['image_ratio'][1] / self.config['image_ratio'][0]):
            ...
            self.config['image_ratio'][1], self.config['image_ratio'][0] = im_width, im_height
            priorbox = PriorBox(self.config, image_size=(self.image_size[1], self.image_size[0]),
                                phase='test')  # height, width
            priors = priorbox.forward()
            if self.config['gpu_inference']:
    ...
    def load_model(self, config, load_to_cpu=True):
        ...
        if load_to_cpu:
            pretrained_dict = torch.load(config['path_pth'], map_location=lambda storage, loc: storage)
        else:
            device = torch.cuda.current_device()
            pretrained_dict = torch.load(config['path_pth'], map_location=lambda storage, loc: storage.cuda(device))
    ...
    def post_process(self, detection_boxes, detection_scores, detection_landmark):
        ...
        boxes = decode(detection_boxes.squeeze(0), self.prior_data, self.config['variance'])
        boxes[:, 0::2] = boxes[:, 0::2] * self.origin_size[1]  # width
        boxes[:, 1::2] = boxes[:, 1::2] * self.origin_size[0]  # height

        landms = decode_landm(detection_landmark.squeeze(0), self.prior_data, self.config['variance'])
